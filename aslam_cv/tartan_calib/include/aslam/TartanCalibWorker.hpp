...
namespace aslam
{
      ...
      public boost::enable_shared_from_this<TartanCalibWorker<C>>
    {
        ...
        private:
            ...
            int minInitCornersAutoComplete = 8; // we need at least this many corners to be able to do autocomplete, since the pose of the board is otherwise too uncertain.
            float minTagSizeAutoComplete = 13; // this is how many pixels a tag needs to be in size before we consider autocompleting it. This is just to make sure really small tags aren't detected and then detected poorly
            float correction_threshold = 20.0; // number of pixel offset between reprojection and detection we allow
            float minResizewindowSize = 3;
